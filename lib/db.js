...
// Use /tmp on Vercel (read-only filesystem), local data/ folder otherwise
const BASE_DIR = process.env.VERCEL === '1' ? '/tmp' : path.join(__dirname, '../data');
const DATA_DIR = path.join(BASE_DIR, 'guides');
const FOLDER_PATH = path.join(BASE_DIR, 'folders.json');
